...
class RobotSequences:
    ...
    def __init__(self, task_manager: TaskManager, printer, arduino):
        self.task_manager = task_manager
        self.printer = printer
        self.arduino = arduino

    def move_to_start(self):
        with self.task_manager.sequence("move_to_start"):
    ...
    def grip_electrodes(self):
        with self.task_manager.sequence("grip_both_electrodes"):
    ...
    def ungrip_both_electrodes(self):
        with self.task_manager.sequence("ungrip_both_electrodes"):
    ...
    def move_to_bath(self):
        with self.task_manager.sequence("move_to_bath"):
            self.task_manager.add_task(Task(
                self.printer.move_to,
                args=(None, None, 120, 4000),
    ...
    def pump_A(self):
        with self.task_manager.sequence("pump_A"):
            self.task_manager.add_task(Task(sleep, args=(1,)))
    ...
    def retract_head(self):
        with self.task_manager.sequence("retract_head"):
            self.task_manager.add_task(Task(
                self.printer.move_to,
                args=(None, None, 120, 4000),
